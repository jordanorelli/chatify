...
class ChatChannel(object):
    ...
    def redis_add_chat_message(self, chat_message):
        ...
        self.redis_server.publish('chat_messages', data)
    ...
    def redis_add_user(self, user):
        ...
        affected = self.redis_server.zadd("users_timestamp",key, user.timestamp)
	...
	data = user.to_json()
        ...
        affected = self.redis_server.zrem('users_timestamp',key)
    ...
    def redis_update_user_timestamp(self, user):
        ...
        data = user.to_json()
        key = "%s:%s" % (self.name, user.nickname)
    ...
    def redis_find_user_by_nickname(self, nickname):
        ...
        key = "users:%s:%s" % (self.channel_id, nickname)
